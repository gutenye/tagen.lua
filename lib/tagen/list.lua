...
local function_arg = tagen.function_arg
local is_type = tagen.is_type
local split = tagen.split
local assert_arg = tagen.assert_arg
local normalize_slice = tablex._normalize_slice

--[[
module ('tagen.list',tagen._module)
]]
...
function List.range(start,finish,incr)
  ...
  if not tagen.is_integer(incr) then finish = finish + eps end
  else
  incr = 1
  end
