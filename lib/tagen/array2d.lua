...
local splitv,fprintf,assert_arg = tagen.splitv,tagen.fprintf,tagen.assert_arg
...
local function makelist (res)
  return setmetatable(res,tagen.stdmt.List)
end
...
function array2d.new(rows,cols,val)
  local res = {}
  local fun = tagen.is_callable(val)
  for i = 1,rows do
    local row = {}
    if fun then
